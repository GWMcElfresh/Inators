...
#' @param vertical_flip Logical, if TRUE, flips the triangles onto the Y axis (default: FALSE).
...
volcanoTriangleInator <- function(df, xmin, xmax, ybase, yheight,
  ...
  if (vertical_flip) {
    # Flip triangles to Y axis: negative X becomes negative Y, positive X becomes positive Y
    triangle_left <- data.frame(
      x = c(ybase, ybase + yheight, ybase),
      y = c(xmin, xmin, 0)
    )
    ...
    #label positions for vertical flip (labels flip with triangles)
    left_label_pos <- data.frame(
      x = (ybase + left_bump_vertical) - 0.1 * yheight,
      y = (xmin + left_bump)
    )
    right_label_pos <- data.frame(
      x = (ybase + right_bump_vertical) - 0.1 * yheight,
      y = (xmax + right_bump)
    )
  } else {
    # Standard horizontal triangles on X axis
    triangle_left <- data.frame(
      x = c(xmin, xmin, 0),
  ...
  # Adjust label justification for vertical flip
  if (vertical_flip) {
    list(
      ...
      annotate("text", x = left_label_pos$x, y = left_label_pos$y, label = left_label, vjust = left_label_hjust, size = label_size, angle = 90),
      annotate("text", x = right_label_pos$x, y = right_label_pos$y, label = right_label, vjust = right_label_hjust, size = label_size, angle = 90)
    )
  } else {
    list(
      geom_polygon(data = triangle_left, aes(x, y), fill = triangle_fill[1], alpha = triangle_alpha, inherit.aes = FALSE),
